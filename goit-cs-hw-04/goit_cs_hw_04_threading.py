...
def search_files(files: list[Path], result_queue: Queue) -> None:
    """Шукає ключові слова у переданій групі файлів."""
...
def run_threading(files: list[Path]) -> dict[str, list[str]]:
    """Запускає потоки та повертає спільний результат."""
    result = {word: [] for word in KEYWORDS}
    groups = divide_files(files, min(4, len(files)))
    result_queue = Queue()

    threads = [
        Thread(target=search_files, args=(group, result_queue))
        for group in groups
    ]
...
def main() -> None:
    ...
    results = run_threading(files)

    print("Багатопотокова версія (threading)")
    print_table(results)
